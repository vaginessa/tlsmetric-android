...
public class ExecuteCommand extends Thread {
	...
	public void run() {
			...
			public void run() {
				try {
					while(true) {
						...
						if (Const.IS_DEBUG) {
							Log.d(Const.LOG_TAG, line);
						}
					}
				} catch (IOException e) {
					Log.w(Const.LOG_TAG, "StreamGobbler couldn't read stream or stream closed");
					if (Const.IS_DEBUG) {
						Log.w(Const.LOG_TAG, "StreamGobbler couldn't read stream or stream closed", e);
					}
				} finally {
					try {
						buffReader.close();
		...
		try {
			if (Const.IS_DEBUG) {
				Log.d(Const.LOG_TAG, "COMMAND: " + command);
			}
