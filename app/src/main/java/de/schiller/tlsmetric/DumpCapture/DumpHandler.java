...
public class DumpHandler {
    ...
    public static void start(){
        File bin = new File(RunStore.getAppContext().getFilesDir(), Const.FILE_TCPDUMP);
        File file = new File(RunStore.getAppContext().getFilesDir(), Const.FILE_DUMP);
        if (!bin.exists()){
            deployTcpDump(RunStore.getAppContext());
        } else {
    ...
    public static void startAnalyzerService(){
        ...
        for(int i = 0; i < 10; i++){
            if(file.exists()) {
                ...
                //Intent intent = new Intent(RunStore.getAppContext(), AnalyserService.class);
    ...
    */
    ...
    public void checkSu() {
        if (RootTools.isRootAvailable()) {
            Toast toast = Toast.makeText(RunStore.getAppContext(), "Superuser is installed.", Toast.LENGTH_LONG);
            toast.show();
        } else {
            Toast toast = Toast.makeText(RunStore.getAppContext(), "Superuser is NOT installed. \n" +
